...
import { useSheetViewModel } from '../viewmodels/useSheetViewModel';
import FAB from '../components/FAB';
import { Expense, HomeStackParamList } from '../types';
import { formatCurrency, formatTaxRate } from '../utils/formatters';

type Props = {
  navigation: NativeStackNavigationProp<HomeStackParamList, 'Sheet'>;
  route: RouteProp<HomeStackParamList, 'Sheet'>;
};
      ...
      >
        <View style={styles.rowLeft}>
          ...
          <Text style={[styles.rowMeta, { color: colors.textSecondary }]}>
            {formatTaxRate(expense.tax_rate)} tax · {expense.split_count}{' '}
            {expense.split_count === 1 ? 'person' : 'people'}
          </Text>
        </View>
        <View style={styles.rowRight}>
          ...
          <Text style={[styles.rowPerPerson, { color: colors.primary }]}>
            {formatCurrency(expense.per_person)}/ea
          </Text>
        </View>
        <Ionicons
...
export default function SheetScreen({ navigation, route }: Props) {
  const { sheetId, sheetName } = route.params;
  const colors = useAppTheme();
  const vm = useSheetViewModel(navigation, sheetId);

  const confirmDelete = (expense: Expense) => {
    Alert.alert(
      'Delete Item',
      `Remove "${expense.item_name}"?`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Delete',
          style: 'destructive',
          onPress: () => vm.deleteExpense(expense.id),
        },
      ],
    );
  };
  ...
  // Sheet total footer
  const sheetTotal = vm.expenses.reduce((s, e) => s + e.total_price, 0);
          ...
          >
            {vm.isExporting ? (
              ...
              <Ionicons
                ...
                size={22}
                color={
                  vm.expenses.length === 0 ? colors.textSecondary : colors.primary
                }
              />
            )}
          </TouchableOpacity>
            ...
            >
              ...
              <View style={styles.modalOptionText}>
                ...
                <Text style={[styles.modalOptionSub, { color: colors.textSecondary }]}>
                  A formatted PDF copy of the sheet
                </Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  listContent: { flexGrow: 1, paddingTop: 8, paddingBottom: 90 },
  row: {
    flexDirection: 'row',
    alignItems: 'center',
